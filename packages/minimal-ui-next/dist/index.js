...
(async function initActual() {
    ...
    try {
        ...
        const dataDir = process.env.ACTUAL_DATA_DIR || path_1.default.join(__dirname, '..', '..', '..', 'data');
    ...
    try {
        ...
        transactions.forEach(transaction => {
            if (transaction.is_child)
                return; // Skip child transactions
...
app.get('/api/transactions/:accountId', ensureBudgetLoaded, async (req, res) => {
    try {
        ...
        const parentTransactions = transactions.filter(t => t.is_parent === true || (t.subtransactions && t.subtransactions.length > 0));
        if (parentTransactions.length > 0) {
            ...
            parentTransactions.forEach(parent => {
                ...
                if (parent.subtransactions && parent.subtransactions.length > 0) {
                    console.log(`  Subtransactions for ${parent.id}:`, parent.subtransactions.map(sub => `${sub.amount} ${sub.category || 'no category'}`).join(', '));
                }
                else {
...
app.delete('/api/categories/:categoryId', ensureBudgetLoaded, async (req, res) => {
    try {
        ...
        await actualAPI.deleteCategory(categoryId, transferCategoryId);
        res.json({
            success: true,
    ...
    try {
        ...
        try {
            ...
            if (createError.message && createError.message.includes('already exists')) {
                ...
                const similarGroup = allGroups.find(g => g.name.toLowerCase() === name.toLowerCase());
                if (similarGroup) {
...
app.delete('/api/category-groups/:groupId', ensureBudgetLoaded, async (req, res) => {
    try {
        ...
        await actualAPI.deleteCategoryGroup(groupId, transferGroupId);
        res.json({
            success: true,
