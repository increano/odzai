...
function showSection(section) {
    ...
    if (section === 'budgets')
        loadBudgets();
    if (section === 'accounts' && isBudgetLoaded)
        loadAccounts();
    ...
    if (section === 'budget' && isBudgetLoaded) {
        loadBudgetMonths().then(() => {
            ...
            if (monthSelect.value)
                loadBudgetMonth(monthSelect.value);
        });
    }
...
function applyTransactionFilters() {
    ...
    if (searchText) {
        filteredTransactions = filteredTransactions.filter(transaction => {
            return ((transaction.payee && transaction.payee.toLowerCase().includes(searchText)) ||
...
function filterTransactions(searchTerm, filterType) {
    ...
    return currentTransactions.filter(transaction => {
        ...
        if (filterType === 'all') {
            return ((transaction.payee || '').toLowerCase().includes(searchTerm) ||
    ...
    organizedTransactions.forEach(transaction => {
        ...
        if (childrenToDisplay.length > 0) {
            ...
            childrenToDisplay.forEach((childTransaction, index) => {
                ...
                if (index === 0)
                    childTr.classList.add('first-child');
                if (index === childrenToDisplay.length - 1)
                    childTr.classList.add('last-child');
                ...
                if (childTransaction.category_name) {
                    ...
                    if (window.allCategories) {
                        ...
                        if (category)
                            childCategoryDisplay = category.name;
                    }
                }
...
async function checkCategoryExists(categoryName, groupId) {
    try {
        ...
        return group.categories.some(cat => cat.name.toLowerCase() === categoryName.toLowerCase() &&
...
function createOrUpdateCategory(categoryId, name, groupId) {
    ...
    const groupOption = document.getElementById('category-group-select').options[document.getElementById('category-group-select').selectedIndex];
